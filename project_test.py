...
def cal_error_rate(attribute_test, label_test, KNN_classfier):
    ...
    return error_rate_KNN, predict_list
...
def cost_func(attribute_train, label_train, attribute_test, label_test, num_neighbors):
    ...
    error_rate, predict_list = cal_error_rate(attribute_test, label_test, KNN_classfier)
    return error_rate
...
def PSO_KNN(X,y, nPOP, MaxIt):
    error_rate, k_list = single_cross(X, y, nPOP, MaxIt)

    k = max(k_list, key=k_list.count)
    print("chosen k: ", k)

    return k

def PSO_KNN_predict(KNN_classfier, attribute_ele):
    predict = KNN_classfier.predict([attribute_ele])
    return predict

def PSO_KNN_cal_error_rate(attribute_test, label_test, KNN_classfier):
    count = 0
    error_KNN = 0
    predict_list = []
    for i in attribute_test:
        predict = PSO_KNN_predict(KNN_classfier, i)
        predict_list.append(predict)
        if (predict != label_test[count]):
            error_KNN += 1
            count += 1
...
if __name__ == "__main__":
    ...
    PSO_KNN_k = PSO_KNN(newData_train, label_train, 4, 30)
    PSO_KNN_classfier = KNN(attribute_test, label_test, PSO_KNN_k)
    error_rate_PSO_KNN, PSO_KNN_predict_list = PSO_KNN_cal_error_rate(attribute_test, label_test, PSO_KNN_classfier)
    print("PSO_KNN accuracy: ", 1 - error_rate_PSO_KNN)

    print("PSO_KNN confusion matrix: ")
    print(confusion_matrix(label_test, PSO_KNN_predict_list))



    KNN_classfier = KNN(attribute_test, label_test, 5)
    error_rate_KNN, KNN_predict_list = cal_error_rate(attribute_test, label_test, KNN_classfier)
    print("Comparative Experiment accuarcy: ", 1 - error_rate_KNN)
    print("Comparative Experiment confusion matrix: ")
    print(confusion_matrix(label_test, KNN_predict_list))
